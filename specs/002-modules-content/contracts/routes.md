...
|---|---|---|---|---|
| GET | `/modules` | auth | none | Every module the caller may see. The list differs by role, and that is the whole point (FR-006/007/008). |
| GET | `/modules/new` | admin | none | The creation form. |
| POST | `/modules` | admin | `title`, `description` | Creates it unpublished. Redirect to its home. |
| GET | `/modules/{id}` | `module:read` | none | Module home: description, published pages in order, module navigation. |
| GET | `/modules/{id}/edit` | admin | none | The edit form. |
...
| POST | `/modules/{id}/delete` | admin | none | Soft-delete. Reversible (FR-010). |
| POST | `/modules/{id}/restore` | admin | none | Undoes it. |
...
| POST | `/modules/{id}/instructors/{uid}/remove` | admin | none | Removes that registration. The module keeps its pages. |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| GET | `/modules/{id}/pages` | `module:write` | none | The instructor's page list, drafts included, in order. |
| GET | `/modules/{id}/pages/new` | `module:write` | none | The editor, empty. |
| POST | `/modules/{id}/pages` | `module:write` | `title`, `body` | **Sanitises `body`, then stores** (FR-016). Created as a draft, positioned last. |
| GET | `/modules/{id}/pages/{pid}/edit` | `module:write` | none | The editor, loaded. |
...
| POST | `/modules/{id}/pages/{pid}/delete` | `module:write` | none | Removes it. Its images stay on the volume. |
| POST | `/modules/{id}/pages/reorder` | `module:write` | `order[]` | Rewrites positions in one transaction (research R5). |
| GET | `/modules/{id}/pages/{pid}` | `module:read` | none | The reading view. Drafts are 404 for a trainee. |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| GET | `/modules/{id}/roster` | `module:write` | none | Everyone registered, and in what capacity (FR-029). |
...
| POST | `/modules/{id}/roster/{uid}/remove` | `module:write` | none | Removes that registration. |
