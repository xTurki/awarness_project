...
|---|---|---|---|---|
| GET | `/modules/{id}/questions` | `module:write` | none | The bank, in order. |
| GET | `/modules/{id}/questions/new` | `module:write` | none | The question form. |
...
| GET | `/modules/{id}/questions/{qid}/edit` | `module:write` | none | The form, loaded. |
| POST | `/modules/{id}/questions/{qid}` | `module:write` | same | Updates it. Refused if the question is in an attempted test (FR-013). |
| POST | `/modules/{id}/questions/{qid}/delete` | `module:write` | none | Removes it. Refused if it is in an attempted test. |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| GET | `/modules/{id}/tests` | `module:read` | none | Instructors see all; trainees see published ones inside their window. |
| GET | `/modules/{id}/tests/new` | `module:write` | none | The test form. |
...
| GET | `/modules/{id}/tests/{tid}/edit` | `module:write` | none | The form. Shows why it is read-only if attempted (FR-014). |
...
| GET | `/modules/{id}/tests/{tid}` | `module:read` | none | Test detail. A trainee sees the time limit, attempts allowed, and pass mark before starting (FR-009). |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| POST | `/tests/{tid}/attempts` | `module:read` | none | Starts an attempt, or resumes the one in progress. Fixes `ends_at` and `question_order`. Refuses outside the window or beyond the attempt limit (FR-015, FR-025). |
| GET | `/attempts/{aid}` | `attempt:own` | none | The attempt: questions in the fixed order, answers already given, `ends_at` for the countdown. |
...
| POST | `/attempts/{aid}/submit` | `attempt:own` | none | Submits and scores. A second submit returns the same result (FR-024). |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| GET | `/tests/{tid}/attempts/mine` | auth | none | The caller's own attempts at that test, each with its date, score, and outcome (FR-030). No path parameter names a person, so there is no route to anyone else's. **Superseded in Phase 3** by `/results/modules/{id}`, which covers every test on the module; it exists so this phase ships complete on its own. |
| GET | `/attempts/{aid}/review` | `attempt:view` | none | Each question with the answer given, whether it was right, and the correct answer (FR-031). Redirects into the attempt if it is still in progress. |
| GET | `/tests/{tid}/attempts` | `module:write` | none | Every attempt on the test, with who, when, score, outcome (FR-033). |
