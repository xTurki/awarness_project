...
|---|---|---|---|---|
| GET | `/login` | anon | none | The sign-in form. A signed-in visitor is redirected to `/`. |
...
| POST | `/logout` | auth | none | Session row deleted, cookie cleared, redirect to `/login`. |
...
| Method | Path | Guard | Sends | Result |
|---|---|---|---|---|
| GET | `/password/new` | auth | none | The choose-a-password form. Redirects to `/` if nothing is required. |
...
|---|---|---|---|
| GET | `/admin/accounts` | none | Every account: email, name, role, active (FR-007). |
| GET | `/admin/accounts/new` | none | The creation form. |
...
| GET | `/admin/accounts/{id}` | none | The edit form. |
